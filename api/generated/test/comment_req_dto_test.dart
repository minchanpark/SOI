//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:soi_api_client/api.dart';
import 'package:test/test.dart';

// tests for CommentReqDto
void main() {
  // final instance = CommentReqDto();

  group('test CommentReqDto', () {
    // int userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // int emojiId
    test('to test the property `emojiId`', () async {
      // TODO
    });

    // int postId
    test('to test the property `postId`', () async {
      // TODO
    });

    // String text
    test('to test the property `text`', () async {
      // TODO
    });

    // String audioKey
    test('to test the property `audioKey`', () async {
      // TODO
    });

    // String waveformData
    test('to test the property `waveformData`', () async {
      // TODO
    });

    // int duration
    test('to test the property `duration`', () async {
      // TODO
    });

    // double locationX
    test('to test the property `locationX`', () async {
      // TODO
    });

    // double locationY
    test('to test the property `locationY`', () async {
      // TODO
    });

    // String commentType
    test('to test the property `commentType`', () async {
      // TODO
    });


  });

}
