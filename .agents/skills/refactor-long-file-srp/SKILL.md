...
1. Plan the refactor before editing.
   - Read the target file and immediate collaborators using `Read`.
   - Run available tests/analyze/lint for the touched area via `Bash`.
   - Map responsibilities and identify extraction candidates.
   - Track extraction steps using `TodoWrite`.
...
3. Apply incremental refactor steps.
   ...
   - Use the `Edit` tool for targeted modifications. Use `Write` only for new files.

4. Control coupling and ownership.
...
5. Verify and finalize.
   - Run static analysis and relevant tests via `Bash`.
